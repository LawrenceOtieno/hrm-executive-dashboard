...
avg_salary = filtered_df['Salary'].mean() if total_headcount > 0 else 0
...
if total_headcount > 0:
    # Compile base aggregates from dataset
    regional_data = filtered_df.groupby('Location').agg(
    ...
    # --- ADDED: REGIONAL OPERATIONAL METRICS SUMMARY GRID ---
    ...
    # Formatted styling loop to add currency notation and clear string labels
    formatted_grid = summary_grid.style.format({
        "Employee Count": "{:,}",
        "Average Salary (KES)": "KES {:,.1f}",
...
# 7. GRANULAR LAYER: INTERACTIVE DATA REFERENCE
st.subheader("🔍 Filtered Employee Roster Reference")
display_df = filtered_df[['EmployeeID', 'FullName', 'Gender', 'Age', 'Department', 'JobTitle', 'Location', 'Salary']].copy()
