...
# CACHED DATA LOADING (Ensures active and inactive tracking)
@st.cache_data
def load_data():
    try:
        ...
        # Generate the 58 inactive employees who left
        for i in range(58):
            data.append({
                "EmployeeID": f"EMP-{len(data)+1000}",
...
# Ensure Status handles active/inactive correctly
...
# KPI CALCULATION POOL
active_workforce = filtered_df[filtered_df["Status"] == "Active"]
inactive_workforce = filtered_df[filtered_df["Status"] == "Left"]

total_headcount = len(active_workforce)
attrition_count = len(inactive_workforce)
...
selected_status = st.segmented_control(
    "Filter Roster View By Status:",
    options=["All Staff", "Active Only", "Left Only"],
    default="All Staff"
)
...
# Clean column headers for executive presentation
display_df = display_df[["EmployeeID", "FullName", "Status", "Gender", "Age", "Department", "JobTitle", "Location", "Salary"]]
display_df.columns = ["Employee ID", "Employee Name", "Employment Status", "Gender", "Age", "Department", "Designation", "Regional Hub", "Annual Salary (KES)"]

# Highlight rows or style values natively
st.dataframe(
