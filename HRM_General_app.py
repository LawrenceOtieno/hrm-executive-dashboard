...
if total_headcount > 0:
    ...
    hub_color_map = {"Mombasa": "#004d40", "Kisumu": "#0288d1", "Nakuru": "#26a69a", "Nairobi": "#78909c"}

    row1_col1, row1_col2 = st.columns(2)
    row2_col1, row2_col2 = st.columns(2)

    with row1_col1:
        st.markdown("<p style=\x27font-size:15px; font-weight:bold; margin-bottom:5px;\x27>?? EMPLOYEE DISTRIBUTION</p>", unsafe_allow_html=True)
        fig_donut = px.pie(regional_data, values="Employee_Count", names="Location", hole=0.4, color="Location", color_discrete_map=hub_color_map)
        fig_donut.update_layout(height=300, margin=dict(l=20, r=20, t=20, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", legend=dict(orientation="h", yanchor="bottom", y=-0.1, xanchor="center", x=0.5))
        fig_donut.update_traces(textposition="inside", textinfo="percent", textfont=dict(size=13, color="white"))
        st.plotly_chart(fig_donut, use_container_width=True, config={"displayModeBar": False})

    with row1_col2:
        st.markdown("<p style=\x27font-size:15px; font-weight:bold; margin-bottom:5px;\x27>?? AVG SALARY COMPARISON</p>", unsafe_allow_html=True)
        fig_sal_bar = px.bar(regional_data, x="Average_Salary_KES", y="Location", orientation="h", color="Location", color_discrete_map=hub_color_map, text="Average_Salary_KES")
        fig_sal_bar.update_layout(height=300, showlegend=False, margin=dict(l=20, r=140, t=20, b=20), xaxis=dict(showgrid=True, gridcolor="#2d3139", title="KES"), yaxis=dict(showgrid=False, title="", categoryorder="total ascending"), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
    ...
    with row2_col1:
        st.markdown("<p style=\x27font-size:15px; font-weight:bold; margin-bottom:5px;\x27>?? TENURE & SALARY RELATIONSHIP</p>", unsafe_allow_html=True)
        fig_relationship = px.scatter(regional_data, x="Average_Tenure_Years", y="Average_Salary_KES", color="Location", color_discrete_map=hub_color_map, text="Location", size="Employee_Count", size_max=30)
        fig_relationship.update_traces(textposition="top center", textfont=dict(size=12, color="white"), marker=dict(line=dict(width=1, color="white")))
        fig_relationship.update_layout(height=320, showlegend=False, margin=dict(l=40, r=40, t=30, b=40), xaxis=dict(title="Average Tenure (Years)", showgrid=True, gridcolor="#2d3139", zeroline=False), yaxis=dict(title="Average Salary (KES)", showgrid=True, gridcolor="#2d3139", tickformat=",.0f", zeroline=False), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_relationship, use_container_width=True, config={"displayModeBar": False})

    with row2_col2:
        st.markdown("<p style=\x27font-size:15px; font-weight:bold; margin-bottom:5px;\x27>?? GEOGRAPHIC HUB LOCATIONS (KENYA)</p>", unsafe_allow_html=True)
    ...
    st.dataframe(summary_grid.style.format({"Employee Count": "{:,}", "Average Salary (KES)": "KES {:,.0f}", "Average Tenure": "{:.1f} Years"}), use_container_width=True, hide_index=True)
...
if "Status" in filtered_df.columns:
    ...
    status_summary["Employment Status"] = status_summary["Employment Status"].map({"Active": "Active Employees", "Left": "Inactive (Left)"}).fillna(status_summary["Employment Status"])
    st.dataframe(status_summary.style.format({"Total Staff Count": "{:,}"}), use_container_width=True, hide_index=True)
